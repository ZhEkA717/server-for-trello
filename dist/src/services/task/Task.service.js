...
const searchTasks = (idBoard, idColumn) => {
    ...
    return (_b = (_a = dataBaseBoards
        .find((board) => board.idBoard === idBoard)) === null || _a === void 0 ? void 0 : _a.columns.find((column) => column.idColumn === idColumn)) === null || _b === void 0 ? void 0 : _b.tasks;
};
...
const createNewTask = (task, idBoard, idColumn) => new Promise((resolve) => {
    const board = dataBaseBoards.find((boardItem) => boardItem.idBoard === idBoard);
    const column = board === null || board === void 0 ? void 0 : board.columns.find((columnItem) => columnItem.idColumn === idColumn);
...
const createNewTaskInColumn = (task, idColumn) => new Promise((resolve, reject) => {
    const board = (0, Board_service_1.searchBoardByColumnID)(idColumn);
    if (board) {
        const column = board.columns.find((columnItem) => columnItem.idColumn === idColumn);
        if (column) {
            const idTask = (0, uuid_1.v4)();
            const newTask = Object.assign(Object.assign({}, task), { idTask });
...
const deleteTaskByIDS = (id) => {
    ...
    if (indexTask === -1)
        throw new CustomErrors_1.NotFoundError('Task not found');
